...
def f_updatephi_pw(f1_curr, K_f1, s1_curr, L1_curr, L1_propvar, Dmat):
    ...
    postlog_prop = (
        -np.log(det(K_f1_tmp)) / 2
        - SSEf_prop
        - np.log(1 - norm.cdf(-L1_curr / L1_propvar))
    )
    postlog_curr = (
        np.log(det(K_f1)) / 2 - SSEf_curr - np.log(1 - norm.cdf(-L1_prop / L1_propvar))
    )
    ratio = np.minimum(1, np.exp(postlog_prop - postlog_curr))
...
def f_logl_pw(g, q1, q2, var1, SSEg):
    ...
    n = q1.shape[0]
    out = (
        n * np.log(1 / np.sqrt(2 * np.pi))
        - n * np.log(np.sqrt(var1))
        - SSEg / (2 * var1)
    )
...
def f_updateg_pw(g_coef_curr, g_basis, var1_curr, q1, q2, SSE_curr, propose_g_coef):
    ...
    SSE_prop = f_SSEg_pw(
        uf.f_basistofunction(g_basis["x"], 0, g_coef_prop["prop"], g_basis), q1, q2
    )
