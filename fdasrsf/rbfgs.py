...
class rlbfgs:
    ...
    def solve(self, maxiter=30, verb=0, lam=0, penalty="roughness"):
        ...
        options = {
            ...
            "maxtime": maxtime,
            "memory": memory,
            "strict_inc_func": strict_inc_func,
        ...
        while True:
            ...
            if stop == 0:
                if stats["stepsize"] < options["minstepsize"]:
                    if not ultimatum:
                        if verb >= 2:
                            print(
                                "stepsize is too small, restarting the bfgs procedure at the current point.\n"
                            )
                        j = 0
                        ultimatum = True
                    else:
            ...
            stepsize, hNext, lsstats = self.linesearch_hint(
            ...
            hNextCost, hNextGradient = self.alignment_costgrad(
            ...
            if inner_sk_sk != 0 and (inner_sk_yk / inner_sk_sk) >= cap:
                ...
                # the cautious step is rejected we do not store sk, yk, and r
                # hok
            else:
                accepted = False
    ...
    def getDirection(
        self, hCurGradient, sHistory, yHistory, rhoHistory, scaleFactor, j
    ):
        """
        BFGS step, see Wen's paper for details. This function takes in a
        tangent vector g, and applies an approximate inverse Hessian P to
        it to get Pg. Then, -Pg is returned. Parallel transport is not
        needed for this problem since we always work in the tangent space
          of identity.
        """
        q = hCurGradient
    ...
    def linesearch_hint(self, d, f0, df0, q2k, lam=0, penalty="roughness"):
        ...
        step does not fulfill the Armijo sufficient decrease criterion, that
        step size is reduced geometrically until a satisfactory step size is
        obtained or until a failure criterion triggers.

        Below, the step is constructed as alpha*d, and the step size is the
        norm of that vector, thus: stepsize = alpha*norm_d. The step is
        executed by computing the exponential mapping exp_{hid}(alpha*d),
        giving newh.
        """
