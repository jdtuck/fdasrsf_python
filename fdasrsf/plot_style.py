...
def rstyle(ax, pres=False):
    ...
    ax.xaxis.set_minor_locator(
        (pylab.MultipleLocator((plt.xticks()[0][1] - plt.xticks()[0][0]) / 2.0))
    )
    ax.yaxis.set_minor_locator(
        (pylab.MultipleLocator((plt.yticks()[0][1] - plt.yticks()[0][0]) / 2.0))
    )
...
def plot_reg_open_curve(beta1, beta2n):
    ...
    for j in range(0, int(T / 5)):
        i = j * 5
        ax.plot(
            array([beta1[0, i], beta2n[0, i]]),
            array([beta1[1, i], beta2n[1, i]]),
            "k",
            linewidth=1,
        )
