...
def optimum_reparam_curve(q1, q2, lam=0.0, method="DP"):
    ...
    if method == "DP":
        gam = orN.coptimum_reparam_curve(
...
def find_best_rotation(q1, q2, allow_reflection=False, only_xy=False):
    ...
    if q1.ndim != 2 or q2.ndim != 2:
        raise Exception(
            "This only supports curves of shape (N,M) for N dimensions and M samples"
        )
...
def find_rotation_and_seed_unique(
    q1, q2, closed=0, lam=0.0, rotation=True, method="DP"
):
    """
...
def elastic_distance_curve(
    beta1, beta2, closed=0, rotation=True, scale=False, method="DP"
):
    """
    ...
    if (beta1 == beta2).all():
        ...
        beta2best, qn_t, Rbest, gam = find_rotation_and_seed_coord(
            beta1, beta2, closed, rotation, method
        )
...
def inverse_exp_coord(beta1, beta2, closed=0, method="DP"):
    ...
    beta2n, q2n, O_hat, gamI = find_rotation_and_seed_coord(
        beta1, beta2, closed, method
    )
