...
class rlbfgs {      
  ...
  public:
    ...
    void solve(int maxiter=30, int verb=0, double lam=0.0, int penalty){
        ...
        lstats lstat;
        vec hNext;
        alignment_costgrad(q2tilde, htilde, lam, penalty, hCurCost, hCurGradient);
        ...
        while (true){
            ...
            alignment_costgrad(q2tilde, hNext, lam, penalty, hNextCost, hNextGradient);
    ...
    void alignment_costgrad(vec q2k, vec h, double lam = 0, int penalty = 0, double f, vec g){
        // roughness
        double pen = 0;
        if (penalty == 0){
            vec time1 = arma::linspace<vec>(0,1,h.n_elem);
