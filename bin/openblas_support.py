...
NIGHTLY_BASE_LOC = "https://anaconda.org/scientific-python-nightly-wheels/openblas-libs"
...
def unpack_windows_zip(fname):
    with zipfile.ZipFile(fname, "r") as zf:
        ...
        lib = [
            ...
            if OPENBLAS_LONG in x
            and x.endswith("a")
            and not x.endswith("dll.a")
            and not x.endswith("dev.a")
        ]
        if not lib:
            return (
                "could not find libopenblas_%s*.a "
